EN_WHITELIST = '0123456789abcdefghijklmnopqrstuvwxyzşığüöç ' # space is included in whitelist
EN_BLACKLIST = '!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~\''

FILENAME = 'drive/seq2seq_chatbot_master/data/twitter/chat.txt'
...
#sys.path.insert(0, 'E:\\my projects\\my_robot\\chatbot\\data\\twitter\\')
...
def process_data():
    ...
    # save them
    np.save('drive/seq2seq_chatbot_master/data/twitter/idx_q.npy', idx_q)
    np.save('drive/seq2seq_chatbot_master/data/twitter/idx_a.npy', idx_a)
    ...
    with open('drive/seq2seq_chatbot_master/data/twitter/metadata.pkl', 'wb') as f:
        pickle.dump(metadata, f)

def load_data(PATH='drive/seq2seq_chatbot_master/data/twitter/'):
...
def split_dataset(x, y, ratio = [0.9, 0.05, 0.05] ):
    # number of examples
    data_len = len(x)
    print("holl data length:", data_len)
